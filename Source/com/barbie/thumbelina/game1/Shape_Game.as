	...
	*/
	...
	public class Shape_Game extends MovieClip {
		...
		private function init():void{
			...
			var square_array:Array = new Array({id:0,color:"pink"},{id:6,color:"purple"});			//pink,purple
			var circle_array:Array = new Array({id:1,color:"purple"},{id:2,color:"red"});			//purple,red
			var diamond_array:Array = new Array({id:3,color:"green"},{id:4,color:"black"}); 		//green,black
			var triangle_array:Array = new Array({id:5,color:"green"},{id:9,color:"yellow"}); 		//green,yellow
			var star_array:Array = new Array({id:7,color:"blue"},{id:8,color:"orange"}); 			//blue,orange
			var rectangle_array:Array = new Array({id:10,color:"red"},{id:11,color:"blue"}); 		//red,blue
			var oval_array:Array = new Array({id:12,color:"pink"},{id:13,color:"oval"}); 			//pink,oval
			var heart_array:Array = new Array({id:14,color:"yellow"},{id:15,color:"orange"}); 		//yellow,orange
			var allShapeArray:Array = new Array(square_array,circle_array,diamond_array,triangle_array,star_array,rectangle_array,oval_array,heart_array);
			var numberShapes:int = allShapeArray.length;
			var randomize_shape_array:Array  = nu.randomUniqueIntArray(0, numberShapes, numberShapes);
			var color5Array:Array = new Array();
			game5_Array = new Array();
			for(var s:int=0;s<numberShapes;s++){
				var z:int = randomize_shape_array[s];
				for(var w:int=0;w<allShapeArray[z].length;w++){
					var sColor:String = allShapeArray[z][w].color;
					var colorIn:Boolean = checkArray(sColor,color5Array);
					if(!colorIn){
					
						game5_Array.push(allShapeArray[z][w].id)
						color5Array.push(allShapeArray[z][w].color);
						break;
					}
				}
				if(color5Array.length == 5) break;
			}
			addChild(shape0);
			addChild(shape1);
			addChild(shape2);
		...
		private function itemSelected(e:Event):void{
			...
			if(e.target.id == correctAns) {
				...
				timer.reset();
				timer.addEventListener(TimerEvent.TIMER_COMPLETE, doEnable);
				timer.start();
				e.target.doOops();
				incorrectFB();
			}
		...
		private function doEnable(e:Event):void{
			timer.removeEventListener(TimerEvent.TIMER_COMPLETE, doEnable);
			enableShapes();
		}
		...
		private function checkArray(_str:String,_sArray:Array):Boolean{
			var found:Boolean = false;
			for (var i:int=0;i<_sArray.length;i++){
				if(_sArray[i] == _str) {
					found = true;
					break;
				}
